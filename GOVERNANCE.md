# GOVERNANCE — Cairn---Preview

**Tier C — Recovery-adjacent.** Rendered by `install-governance.sh` from
...
## Binding invariant checklist (Tier C)
...
### Tier C — Recovery-adjacent (adds to A/B)
- [ ] **T-1 Two-registers anti-outing** — public surfaces never name recovery/any member; specifics live behind auth. _(gate + test)_
- [ ] **T-2 Money decoupled from progress** — payments never gate/accelerate progress; no gambling; no pay-to-recover. _(test: two-user behavioral guard)_
- [ ] **T-3 Attraction, not promotion** — no dark patterns / streak-shaming; gentle failure states. _(hook: dark-pattern wordlist lint + PR copy/UX checklist)_
- [ ] **T-6 Consent before communal content** — member stories reach all-members surfaces only via the full six-gate pipeline; unbuilt gates => surface stays closed. _(gate + test)_
- [ ] **T-7 Crisis pathway** — crisis check on EVERY user-text ingress, fires before any freeze/lock, returns help resources in-response. _(middleware + test)_
...
- State: **UNAUDITED.** Checklist rendered for Tier C; no invariant verified yet.
